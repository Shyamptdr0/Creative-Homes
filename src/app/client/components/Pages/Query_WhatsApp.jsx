...
import { Loader2, ChevronDown, ChevronUp, X, Send, Paperclip, CheckCircle, Clock, MessageCircle } from "lucide-react";
...
export default function ClientQueriesPage() {
	...
	if (token) {
		try {
			...
			clientId = decoded.id;
			console.log("CLIENT QUERY - Extracted clientId:", clientId);
		} catch (err) {
			console.error("CLIENT QUERY - TOKEN DECODE ERROR:", err);
		}
	} else {
		console.log("CLIENT QUERY - No token found");
	}

	const fetchProjects = async () => {
		try {
			console.log("CLIENT QUERY - Fetching projects with token:", !!token);
			const res = await fetch("/api/clients/projects", {
				headers: { Authorization: `Bearer ${token}` },
			});
			const data = await res.json();
			console.log("CLIENT QUERY - Projects API response:", data);

			if (data.success && Array.isArray(data.projects)) {
				setProjects(data.projects);
				console.log("CLIENT QUERY - Projects loaded:", data.projects.length);
			} else {
				console.log("CLIENT QUERY - No projects found or API error");
				setProjects([]);
			}
		} catch (error) {
	...
	const fetchQueries = async () => {
		try {
			console.log("CLIENT QUERY - Fetching queries with token:", !!token);
			const res = await fetch("/api/clients/queries", {
				headers: { Authorization: `Bearer ${token}` },
			});

			let data = await res.json();
			console.log("CLIENT QUERY - Queries API response:", data);
			
			if (!data.success) {
				console.log("CLIENT QUERY - No queries found or API error");
				return setQueries([]);
			}

			const sorted = data.queries.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
			setQueries(sorted);
			console.log("CLIENT QUERY - Queries loaded:", sorted.length);
	...
	useEffect(() => {
		if (token && clientId) {
			...
			// If token exists but clientId is null, set loading to false after a short delay
			setTimeout(() => {
				if (!clientId) {
					setLoading(false);
				}
