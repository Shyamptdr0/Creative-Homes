	...
	return (
		<div className="p-8 min-h-screen bg-gradient-to-br from-gray-100 to-gray-50">
				...
				<Dialog open={true}>
					<DialogContent className="max-w-lg text-center space-y-4">
						...
						<div className="p-4 bg-gray-100 rounded-md text-left">
							...
							<p><b>Rate:</b> â‚¹ {pendingApprovalProject.totalCost}</p>
						</div>
          ...
          border border-orange-200 hover:scale-[1.01] transition"
										...
										{paginatedData.length === 0 ? (
											...
											paginatedData.map((p, i) => (
												<TableRow key={i} className="hover:bg-gray-50 transition">
													...
													<TableCell>â‚¹ {p.totalCost || "-"}</TableCell>
													...
													<TableCell className="flex gap-2 justify-center">
														<div className="relative inline-block">
															...
															{/* ðŸ”¥ UNREAD PROJECT BADGE (SUM OF ALL STAGES) */}
			...
			{/* ----------------------------------------------------- */}
			{/* PROJECT SHEET â€” FLOORWISE STAGES + CHAT PANEL */}
			{/* ----------------------------------------------------- */}
			{openProjectSheet && (
				<Sheet
					open={true}
										...
										<div key={floor} className="mb-6 relative">
											...
											<div className="relative pl-8 space-y-7 mt-4">
												...
												{stages.map((s, index) => {
													...
													const icon = (() => {
														if (isApproved)
															return <div className="h-7 w-7 rounded-full bg-green-600 text-white flex items-center justify-center shadow">âœ“</div>;
														if (isCompleted)
															return <div className="h-7 w-7 rounded-full bg-blue-600 text-white flex items-center justify-center shadow">âœ“</div>;
														if (isRejected)
															return <div className="h-7 w-7 rounded-full bg-red-600 text-white flex items-center justify-center shadow">âœ—</div>;
														if (isCurrent)
