...
export default function AdminQueriesPage() {
	...
	if (token) {
		try {
			...
			console.error("ADMIN QUERY - TOKEN DECODE ERROR:", err);
		}
	}
	...
	const fetchProjects = async () => {
		try {
			...
			console.error("ADMIN QUERY - PROJECT FETCH ERROR:", error);
		}
	};
	...
	const fetchQueries = async () => {
		try {
			...
			console.error("ADMIN QUERY - QUERY FETCH ERROR:", error);
			setQueries([]);
		}
	};
	...
	const loadProjectMessages = (project) => {
		...
		projectQueries.forEach(query => {
			// Original message from client, contractor, or admin
			if (query.clientId) {
				messages.push({
					id: query.id,
	...
	const markIssuesAsViewed = async (projectId) => {
		try {
			const res = await fetch(`/api/queries/mark-viewed/${projectId}`, {
				method: 'PATCH',
				headers: { 
			...
			if (res.ok) {
				fetchQueries(); // Refresh queries to update status
			}
		} catch (error) {
			console.error("Error marking issues as viewed:", error);
		}
	};

	const markIssuesAsResolved = async (projectId) => {
		try {
			const res = await fetch(`/api/queries/mark-resolved/${projectId}`, {
				method: 'PATCH',
				headers: { 
					'Authorization': `Bearer ${token}`,
					'Content-Type': 'application/json'
			...
			if (res.ok) {
				...
				formData.append("adminId", adminId);
				formData.append("message", "This issue has been resolved. No further messages can be sent on this topic.");
				formData.append("senderType", "admin");
				formData.append("senderId", adminId);
				...
				fetchQueries(); // Refresh queries to update status
				loadProjectMessages(selectedProject); // Refresh messages
			}
		} catch (error) {
	...
	const selectProject = (project) => {
		...
		// Mark issues as viewed when admin selects a project
		const projectId = projects.find(p => p.title === project)?.id;
		if (projectId) {
			markIssuesAsViewed(projectId);
		}
							...
							<div className="flex items-center justify-between">
								...
								<div className="flex items-center gap-2">
									{issuesByProject[selectedProject]?.some(q => q.status === 'in-progress') && (
										<Button
											onClick={() => {
												const projectId = projects.find(p => p.title === selectedProject)?.id;
												if (projectId) {
													markIssuesAsResolved(projectId);
												}
											}}
